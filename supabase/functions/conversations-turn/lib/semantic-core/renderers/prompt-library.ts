...
export function buildSequenceOfferQuestion(attendeeName?: string): string {
  return `Você quer agendar ${attendeeName || "a próxima pessoa"} na sequência do atendimento anterior? O próximo horário está disponível. Prefere esse horário, outro horário no mesmo dia ou em outro dia?`
}
...
export function buildNextAttendeePrompt(plan: SemanticPostConfirmationPlan): string {
  const suggestionLine =
    plan.suggested_next_time && plan.suggested_next_date
      ? `Posso te sugerir ${plan.suggested_next_time} para ${formatBookingDateForDisplay(plan.suggested_next_date)} e seguir na sequência.`
      : ""
