...
export function determineNextStep(
  ...
  if (
    ...
    return {
      step: 'services_list',
      message:
        'Quais os serviços que seu cliente pode agendar? Liste separando por vírgula.\n\nExemplo: Corte de cabelo, Barba, Corte + Barba',
    }
  }
  ...
  if (
    ...
    if (missing.includes('schedule.days_of_week')) {
      return {
        step: 'schedule_days',
        message:
          'Quais dias da semana você atende?\n\n• Segunda a Sexta\n• Segunda a Sábado\n• Todos os dias\n• Personalizado',
  ...
  if (
    ...
    return {
      ...
      message:
        'Para calcular o orçamento, quais informações você precisa do cliente?\n\nExemplo: medidas (largura, altura), quantidade, tipo de material, etc.',
    }
  }
  ...
  if (!currentData.policies) {
    return {
      step: 'policies',
      message:
        'Você tem alguma política de cancelamento ou sinal? (ex: 50% de sinal, cancelamento com 24h de antecedência)',
    }
  }
  ...
  if (!currentData.faq || currentData.faq.length === 0) {
    return {
      step: 'faq_offer',
      message:
        'Ótimo! Agora, para que eu possa responder dúvidas dos seus clientes de forma inteligente, você gostaria de adicionar algumas perguntas frequentes?\n\nIsso ajuda a IA a responder automaticamente quando seus clientes perguntarem algo similar.',
